...
bool UART1_DMA_RxInit(void* RxBufAddr, uint16_t RxBufLen, uint16_t ThresholdLen, INT_FUNC CallBack)
{
    ...
    UART_IOCtl(UART_PORT1, UART_IOCTL_DMA_RX_EN, 1);
    return TRUE;
}
...
bool UART1_DMA_BlockTxInit(INT_FUNC CallBack)
{
    ...
    UART_IOCtl(UART_PORT1,UART_IOCTL_DMA_TX_EN, 1);
    return TRUE;
}
...
uint32_t UART0_DMA_Send(uint8_t* SendBuf, uint16_t BufLen, uint32_t TimeOut)
{
    ...
    while(!UART_IOCtl(UART_PORT0,UART_IOCTL_TX_FIFO_EMPTY, 0));
    return BufLen;
}
...
uint32_t UART1_DMA_Send(uint8_t* SendBuf, uint16_t BufLen, uint32_t TimeOut)
{
    ...
    while(!UART_IOCtl(UART_PORT1,UART_IOCTL_TX_FIFO_EMPTY, 0));
    return BufLen;
}
...
bool UART0_DMA_TxIsTransferDone(void)
{
	...
	if(DMA_CircularDataLenGet(PERIPHERAL_ID_UART0_TX) == 0 && UART_IOCtl(UART_PORT0,UART_IOCTL_TX_FIFO_EMPTY, 0))
		return TRUE;
	else
		return FALSE;
}
...
{
	/*DMA BUFFER--->UART FIFO--->*/
	if(DMA_CircularDataLenGet(PERIPHERAL_ID_UART1_TX) == 0 && UART_IOCtl(UART_PORT1,UART_IOCTL_TX_FIFO_EMPTY, 0))
		return TRUE;
	else
		return FALSE;
}
...
bool UART0_DMA_RxIsTransferDone(void)
{
	...
	if(DMA_CircularDataLenGet(PERIPHERAL_ID_UART0_RX) == 0 && UART_IOCtl(UART_PORT0,UART_IOCTL_RX_FIFO_EMPTY, 0))
		return TRUE;
	else
		return FALSE;
}
...
{
	/*UART FIFO--->DMA BUFFER--->*/
	if(DMA_CircularDataLenGet(PERIPHERAL_ID_UART1_RX) == 0 && UART_IOCtl(UART_PORT1,UART_IOCTL_RX_FIFO_EMPTY, 0))
		return TRUE;
	else
		return FALSE;
}
