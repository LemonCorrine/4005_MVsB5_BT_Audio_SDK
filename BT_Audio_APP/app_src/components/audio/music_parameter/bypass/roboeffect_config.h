/***************************************************
 * @file    roboeffect_config.h                      
 * @brief   auto generated                          
 * @author  ACPWorkbench: 3.9.2                  
 * @version V1.1.0                                 
 * @graphics: bypass   
 * @Effect Version: 2.38.0
 * @RoboEffect Version: 2.16.2
 * @Created 2024-03-12T10:24:33                                      
 * @copy; Shanghai Mountain View Silicon Technology Co.,Ltd. All rights reserved.
 ***************************************************/

#include "stdio.h"
#include "type.h"

#define    AUTO_TUNE_ENABLE                              (0)
#define    DC_BLOCKER_ENABLE                             (0)
#define    DRC_ENABLE                                    (0)
#define    ECHO_ENABLE                                   (0)
#define    EQ_ENABLE                                     (0)
#define    NOISE_SUPPRESSOR_EXPANDER_ENABLE              (0)
#define    FREQ_SHIFTER_ENABLE                           (0)
#define    HOWLING_SUPPRESSOR_ENABLE                     (0)
#define    NOISE_GATE_ENABLE                             (0)
#define    PITCH_SHIFTER_ENABLE                          (0)
#define    REVERB_ENABLE                                 (0)
#define    SILENCE_DETECTOR_ENABLE                       (0)
#define    THREE_D_ENABLE                                (0)
#define    VIRTUAL_BASS_ENABLE                           (0)
#define    VOICE_CHANGER_ENABLE                          (0)
#define    GAIN_CONTROL_ENABLE                           (1)
#define    VOCAL_CUT_ENABLE                              (0)
#define    REVERB_PRO_ENABLE                             (0)
#define    VOICE_CHANGER_PRO_ENABLE                      (0)
#define    PHASE_CONTROL_ENABLE                          (0)
#define    VOCAL_REMOVER_ENABLE                          (0)
#define    PITCH_SHIFTER_PRO_ENABLE                      (0)
#define    VIRTUAL_BASS_CLASSIC_ENABLE                   (0)
#define    PCM_DELAY_ENABLE                              (0)
#define    HARMONIC_EXCITER_ENABLE                       (0)
#define    CHORUS_ENABLE                                 (0)
#define    AUTO_WAH_ENABLE                               (0)
#define    STEREO_WIDENER_ENABLE                         (0)
#define    PINGPONG_ENABLE                               (0)
#define    THREE_D_PLUS_ENABLE                           (0)
#define    SINE_GENERATOR_ENABLE                         (0)
#define    NOISE_SUPPRESSOR_BLUE_ENABLE                  (0)
#define    FLANGER_ENABLE                                (0)
#define    FREQ_SHIFTER_FINE_ENABLE                      (0)
#define    OVER_DRIVE_ENABLE                             (0)
#define    DISTORTION_DS1_ENABLE                         (0)
#define    EQ_DRC_ENABLE                                 (0)
#define    AEC_ENABLE                                    (0)
#define    COMPANDER_ENABLE                              (0)
#define    LOW_LEVEL_COMPRESSOR_ENABLE                   (0)
#define    BEAT_TRACKER_ENABLE                           (0)
#define    ENGINE_SOUND_ENABLE                           (0)
#define    BIQUAD_ENABLE                                 (0)
#define    CHORUS2_ENABLE                                (0)
#define    REVERB_PLATE_ENABLE                           (0)
#define    HOWLING_SUPPRESSOR_FINE_ENABLE                (0)
#define    HOWLING_GUARD_ENABLE                          (0)
#define    VIRTUAL_BASS_TD_ENABLE                        (0)
#define    OVER_DRIVE_PLOY_ENABLE                        (0)
#define    TREMOLO_ENABLE                                (0)
#define    BEAMFORMING_ENABLE                            (0)
#define    VIRTUAL_SURROUND_2CH_ENABLE                   (0)
#define    ROBOT_TONE_ENABLE                             (0)
#define    NOISE_GENERATOR_ENABLE                        (0)
#define    NOISE_SUPPRESSOR_BLUE_DUAL_ENABLE              (0)
#define    FADER_ENABLE                                  (0)
#define    DOWNMIX_2TO1_ENABLE                           (0)
#define    UPMIX_1TO2_ENABLE                             (0)
#define    CHANNEL_COMBINER_ENABLE                       (0)
#define    CHANNEL_SELECTOR_ENABLE                       (0)
#define    ROUTE_SELECTOR_ENABLE                         (0)
#define    AI_DENOISE_ENABLE                             (0)
#define    SIMPLE_GAIN_ENABLE                            (0)
