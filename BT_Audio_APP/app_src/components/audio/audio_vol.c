...
extern volatile int16_t gPcmZeroValue;
extern volatile uint16_t gPcmZeroCounActionPoint;
volatile uint32_t gFindPcmZeroFlag = 0;
static uint32_t sFindPCM0TimeOut = 0;
static uint32_t sFindPCM0TimeOutActionPoint =  500;
void SetFindPCMZeroStart(void)
{
	if(GetSystemMode() != ModeIdle&&(AudioCoreSourceIsEnable(APP_SOURCE_NUM)
#ifdef CFG_FUNC_REMIND_SOUND_EN
//			|| get_remind_state() == MP2_DECODE_FRAME
#endif
			))
	{
		if(GetSystemMode() == ModeBtHfPlay)//|| get_remind_state() == MP2_DECODE_FRAME
		{
			gPcmZeroValue = 1;
			gPcmZeroCounActionPoint = 10;
		}
#ifdef CFG_FUNC_REMIND_SOUND_EN
		else if(get_remind_state() == MP2_DECODE_FRAME)
		{
			gPcmZeroValue = 1;
			gPcmZeroCounActionPoint = 1;
		}
#endif
		else
		{
			gPcmZeroValue = 1;
			gPcmZeroCounActionPoint = 0;
		}
		
		sFindPCM0TimeOutActionPoint = 500;
		if(GetSystemMode() == ModeLineAudioPlay)
		{
			sFindPCM0TimeOutActionPoint = 300;
		}
		
		gFindPcmZeroFlag = 1;
		sFindPCM0TimeOut = 0;
		while(gFindPcmZeroFlag && (sFindPCM0TimeOut < sFindPCM0TimeOutActionPoint))
		{
			if(sFindPCM0TimeOut >sFindPCM0TimeOutActionPoint-2)
			{
				APP_DBG("find 0 time out\n");
			}
			sFindPCM0TimeOut++;
			vTaskDelay(1);
		}
		AudioDAC_FadeTimeSet(DAC0, 20);
		gFindPcmZeroFlag = 0;
	}
}
#endif
...
void HardWareMuteOrUnMute(void)
{
	...
	{	
		...
		if(GetSysModeState(GetSystemMode()) !=  ModeStateDeinit)
		{
			APP_DBG("DigitalMute DAC0\n");

			AudioDAC_DigitalMute(DAC0, TRUE, TRUE);

		}
		else //if((*(volatile unsigned long *) 0x4002E000) & 0x01)
		{
			APP_DBG("SoftMute DAC0\n");
		...
		if(GetSysModeState(GetSystemMode()) !=  ModeStateDeinit)
		{
			APP_DBG("DigitalMute DAC1\n");
			AudioDAC_DigitalMute(DAC1, TRUE, TRUE);
		}
		else //if((*(volatile unsigned long *) 0x4002E024) & 0x01)
		{
			APP_DBG("SoftMute DAC1\n");
		...
		I2S_MuteSet(CFG_RES_I2S_MODULE,TRUE);
#endif
#endif // if 0
	}
	else
...
#ifdef CFG_RES_AUDIO_I2SOUT_EN
		I2S_MuteSet(CFG_RES_I2S_MODULE,FALSE);
#endif
#endif //if 0
	}
