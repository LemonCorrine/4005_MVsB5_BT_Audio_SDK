...
{
	memset(&gBtHfCt->AecDelayBuf[0], 0, (AEC_BLK_LEN*LEN_PER_SAMPLE*MAX_DELAY_BLOCK));
	
	gBtHfCt->AecDelayRingBuf.addr = &gBtHfCt->AecDelayBuf[0];
	...
	memset(&gBtHfCt->SourceBuf_Aec[0], 0, CFG_BTHF_PARA_SAMPLES_PER_FRAME * 2);
	return TRUE;
}
...
int16_t *BtHf_AecInBuf(void)
{
	...
	{
		BtHf_AECRingDataGet(&gBtHfCt->SourceBuf_Aec[0], CFG_BTHF_PARA_SAMPLES_PER_FRAME);
	}
	else
	{
		memset(&gBtHfCt->SourceBuf_Aec[0], 0, CFG_BTHF_PARA_SAMPLES_PER_FRAME * 2);
	}
	return (int16_t *)&gBtHfCt->SourceBuf_Aec[0];
}
#endif
/*******************************************************************************
 * hf volume sync
